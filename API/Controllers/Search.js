...
export const searchFilms = async (req, res) => {
  try {
    ...
    {
      ListFilms = await Film.find(
        {$or: [{$and: [{"crew.name":  {$regex : regex} }, {"crew.job": "Director"}]}, {"cast.name":  {$regex : regex}}]
        },
        projection
      ).limit(20);
    }
    ...
    {
      let listFind = [await Recommendation.findOne({id: ListFilms[index].id}, {"recommendation.id": 1})];

      let arr = []

      for(var item of listFind[0].recommendation)
      {
        if(ListFilms.length + arr.length > 20)
          break;
        arr.push(item.id)
      }

      ListFilms.push(...await Film.find({id: {$in: arr}}, 
        { title: 1,
        poster_path: 1,
        id: 1,release_date: 1 }));
